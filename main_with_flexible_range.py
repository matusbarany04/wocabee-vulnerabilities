...
class wocabot:
    def __init__(self,username,password,args):
        ...
        self.dictionary_path = "./wocabeearchive/8.A.json"
    ...
    def pick_package(self,package_id,packages):
        ...
        packages[-1][list(packages[-1].keys())[0]].click()
    ...
    def practice(self,target_wocapoints = None):
        ...
        target_wocapoints = int(target_wocapoints) + random.randint(-100,100)

        if target_wocapoints < wocapoints:
            target_wocapoints = wocapoints + 10
    ...
    def dictionary_get(self, word, *args, **kwargs):
        ...
        words = [x.strip() for x in word.split(",")] if "," in word else [word]
        end = []
        ...
        for word in words:
            if word in dictionary:
                ...
                if isinstance(translation, list):
                    end.extend(translation)
                else:
                    end.append(translation)

        return end if end else [word]
